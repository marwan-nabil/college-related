...
m = input('enter the number of samples per bit m >>'); % samples per waveform
T = input('enter the sampling period T >>'); % sampling instant
S1(1:m) = input('enter the waveform S1 as an array >>');
S2(1:m) = input('enter the waveform S2 as an array >>'); % the two known waveforms
...
sampled_filtered = filtered_signal(T : m : end); % sampling
% comparator
